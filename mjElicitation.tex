...
\commentOC{Could also write $\restr{P}{S × N}$, which leads to more generality, if restricting the users is also sometimes convenient. For this definition you do not need $S ≠ \emptyset$, I’d remove this restriction. You could write $\emptyset ≠ S \subseteq A$ instead of restricting on the cardinal.}

Given any $s \in \intvl{1,m}$, a grading function $f^s: \Delta^{s \times n} \rightarrow \Delta^s$ is a function that assigns to a profile $P^S$ a vector of final grades, one for each alternative. 
\commentOC{to any profile $P^S \in …$.}
...
%\end{align}
The \emph{majority-grade}, $f^s_{maj}$, is a SGF
\commentOC{… is the SGF, I suppose} 
that associates to a profile $P^S$ a vector of median grade values, one for each alternative: $f^s_{maj}(P^S)_i = \rho(P^S_i)_{\lfloor \frac{n}{2} \rfloor + 1}$, $\forall i \in S$. 
\commentOC{This requires $n ≠ 0$, which should probably be indicated from the start.}
...
\commentOC{The codomain is incorrect as $F^s$ selects a set of alternatives, not one alternative (if I understand correctly).}
...
\commentOC{It is arguable that this is the most natural definition, in case of even $n$, as there are two medians that could be used. I’d find it natural to average them (equivalently, use the other one in case of ties). Is this really the way the authors define it? If so, a short sentence about this would be welcome, to make it clear that this is not an oversight.}
Let $h$ denote the highest median grade, $h=\max_{i\in S}\rho(P^S_i)_{\lfloor \frac{n}{2} \rfloor + 1}$, and consider the case where two, or more, alternatives are selected as winners \textemdash i.e. candidates with the same highest median grade $h$. We break ties 
\commentOC{The phrasing “We break ties” lets the reader suspect that you choose to introduce this. Please specify whether this is the original definition, or rephrase.}
by removing one $h$ grade from the vectors of grades of each tied alternative, recomputing the new median grade and repeating the process until one unique winner is found or there are no more grades to remove. 
\commentOC{Isn’t this procedure equivalent to considering the next highest grade, and so on, lexicographically? I think this description is a better explanation of the spirit of the procedure. (And this probably cancels my previous remark about ties.)}
...
\commentOC{Perhaps you could just define an ordering over the vector of grades. (This may not be a good suggestion, depending on what you want to do with it.)}
...
\commentOC{Do you mean $K_j \subseteq A$?}
...
\commentOC{Consider using the \LaTeX command \emph{suchthat} (defined in my template).}
...
\commentOC{I do not understand this: how do we know the best $k$ alternatives, before asking?}
...
\commentOC{It should be \emph{we need}, not \emph{then}. Alternatively: $w \in W^*$ requires $…$.}
...
\commentOC{A notation for the $x$th median grade or $x$th best alternative would be useful.}

Because the voters express their judgments on all the alternatives in $\tilde{K}$ 
\commentOC{The reason for this claim is unclear to me. Is this a deduction, or an hypothesis?}
