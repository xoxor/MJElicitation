...
\end{theorem}
\begin{proof}
	Consider the complete profile $P$ defined in the proof of \Cref{th:notinK}, we will show how to construct an incomplete profile for any value of $k\in \intvl{1,m-1}$ such that the statement is true. Let us call $K=\overline{F}_k(\Pbar)$.
	...
	The grade vector of $j_1$ is $\Pbar_{j_1}=(\dbar, \dots, \dbar)$ and $\fmajbar(\Pbar_{j_1})=\dbar$ \commentOC{$\fmajbar(\Pbar_{j_1})$ is undefined, it seems to me}\commentBN{I think it's the $P^k$ that is confusing, it's just a name not a restriction. I wanted to make explicit the relation between $\Pbar$ and $k$. I removed that}.
...
\end{remark}
This observation comes from the proof of \Cref{pr:probabilityJ}. Given an alternative $j\in A$ and a voter $i\in N$ the probability that $i$ is asked about $j$, thus that $\Pbar_{j}(i)$ is defined, is $\frac{k}{m}$. This gives us an approximation on the number of elements of the grade vector, because for each of the $n$ elements, we have $\frac{k}{m}$ probability that the element is defined.
