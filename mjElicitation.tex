...
A \textit{common language} $\Delta = \{ \delta_1, \delta_2, \dots \}$ is a set of strictly ordered grades. It may, or may not, be finite and the notation $\delta_1 \geq \delta_2$ indicates that $\delta_1$ is a better or equivalent grade than $\delta_2$. A profile $P : A\times N \rightarrow \Delta$ is a $m$ by $n$ matrix of grades $P \in \Delta^{A \times N}$. The operator $\rho: \Delta^{N} \rightarrow \Delta^n$ defines an ordering function that given a vector of grades $P_i$ returns the vector ordered by decreasing grades.
...
The \emph{majority-grade}, $f_{maj}$, is the function that associates to a vector of grades $q \in \Delta^N$, with $|q|\neq 0$, its median grade value: $f_{maj}(q) = \rho(q)_{\lfloor \frac{|q|}{2} \rfloor + 1}$. Given any $S\subseteq A$, by applying $f_{maj}$ to all vectors of grades associated to the alternatives in $S$ we obtain the corresponding SGF $f^S_{maj}$. The $i$-\emph{th} element of the resulting vector is the median of the ordered vector of grades associated to the $i$-\emph{th} alternative. Because $f^S_{maj}(P^S)_i$ must depend only on $P^S_i$ we write $f^S_{maj}(P^S_i)$. Moreover, when we consider the complete profile (when $S=A$) we will write $f_{maj}(P_i)$ instead of $f^A_{maj}(P^A_i)$.
...
is a function that selects the alternative with the highest median grade as winner: $F^S_{maj}(P^S) = \argmax_{i\in S}\rho(P^S_i)_{\lfloor \frac{n}{2} \rfloor + 1}$. Note that in case $n$ is even two medians could be used for an alternative, but the lower grade is picked.

Let $h$ denote the highest median grade associated to the alternatives in $S$, $h=\max_{i\in S}\rho(P^S_i)_{\lfloor \frac{n}{2} \rfloor + 1}$, and consider the case where two, or more, alternatives could be selected as winners \textemdash i.e. candidates with the same highest median grade $h$. Ties are broken by removing one $h$ grade from the vectors of grades of each tied alternative, recomputing the new median grade and repeating the process until one unique winner is found or there are no more grades to remove. When $n$ is odd, this is equivalent to take the next element after the median, i.e. the one at index $(\lfloor \frac{n}{2} \rfloor + 1) +1$, if there is still a tie we then look at the previous element before the median, i.e. the one at index $(\lfloor \frac{n}{2} \rfloor + 1) -1$, and keep alternating until the tie is broken or there are no more elements in the vector. When $n$ is even the process is similar but we alternate starting from the element before the median, $\lfloor \frac{n}{2} \rfloor$, then the one after, $\lfloor \frac{n}{2} \rfloor + 2$, and so on.
...
Thus $F^S_{maj}$ can also be defined as $F^S_{maj}=i\in S$ such that $i > j, \forall j \in S \setminus \{ i \}$. For brevity, we will write $F_{maj}$ when considering $S=A$.
...
After having asked every voter in $N$ to judge $k$ candidates, we obtain an \emph{incomplete profile} $\overline{P}\in \overline{\Delta}^{A \times N}$, that is a matrix $m \times n$ of grades of which $k \times n$ are "defined". Note that when $k=n$ the resulting profile $\overline{P}$ corresponds to the complete profile $P$. Let $C(\overline{P}) = \{P' \in \Delta^{m \times n} \suchthat \overline{P} \subseteq P'\}$ be the set of all completions of $\overline{P}$ obtained by substituting all \emph{Undefined} grades with "defined" ones, note that $P \in C(\overline{P})$.
