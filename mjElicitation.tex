...
\commentOC{It seems you never use the identity of the alternatives (the content of $S$), just their number ($s$), thus you could probably simplify by using $s$ instead of $S$ in many places. Independently, the notation $P^S$ is a bit strange as it does not in fact in general represent an association of a profile and a set, but simply a matrix in $\Delta^{s × n}$ (which does not require a set $S$ to start with). I’d rather use always the letter $P$ to denote a profile of any size, and write $P \in \Delta^{s × n}$ to specify its size when required.}
