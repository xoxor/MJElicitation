...
\commentOC{I am not sure, but it seems to me a waste of time and space to describe this with words, whereas you will anyway need to define it again more precisely using appropriate formalism if you need to do anything serious with these definitions. And if you don’t need to, then such level of details is probably not justified.}

\section{Notation}
Let us formally define the problem. \commentOC{The reader ignores, at this stage, what “the problem” is, informally speaking. That should stand in the introduction.}
...
A \textit{common language} $\Delta = \{ \alpha, \beta, \dots \}$ is a set of strictly ordered grades. It may, or may not, be finite and the notation $\alpha \geq \beta$ indicates that $\alpha$ is a better or equivalent grade than $\beta$. A profile $P = P(A,N) = \Delta^{m \times n}$ is a $m$ by $n$ matrix of grades. A row $P_i$, $i\in \intvl{1,m}$, represents the ordered vector of grades $(r_1 , \dots, r_n )$ associated to the alternative $i$. \commentOC{That’s a bit confusing as I’d very much expect that $P_{i, j}$ represents the grade given by voter $j$. (Also, I suspect this will be useful in the analyzis.) So I’d expect that $r$ represents the ordered vector but $P$ doesn’t, which is not what the text states. Is this what you meant? Also, it would be nice to insist on the “ordered” aspect of it: as it stands, it is easy to miss that one word, but it is very important. You could write: … represents the vector…, ordered by increasing grade.}
The double brackets denote an interval in the integers. 

A grading function $f: P \rightarrow \Delta^m$ is a function that assigns to a profile $P$ a vector of final grades, one for each alternative. 
In \citet{Balinski2007} definition \commentOC{It’s the definitions that you choose that count here, as this is your article. We want to know what a SGF is in \emph{this} article. Whether their definition say this or that about this or that concept is irrelevant. No need to say, also, that some of the definitions you use such as this one come from their article, as you have cited it already and this definition is not especially remarkable.} , in order to be a social grading function, $f$ must be neutral, anonymous, unanimous, monotonic, independent of irrelevant alternatives and continuous. 
...
%\end{align}
The \emph{majority-grade}, $f_{maj}$, is a social grading function that associates to each row $i$ of the profile $P$ its median grade value. With a small abuse of notation we can define the value $f_{maj}(P_i)$ \commentOC{Please state it first without abuse of notation, that is, $f_\mathit{maj}(P)_i$. You can also observe that as it is IIA, it must depend only on $P_i$ (if this is true), or simply, observe that it does depend only on $P_i$, and thus say that you also write $f_{maj}(P_i)$ instead of $f_{maj}(P)_i$, if you find this convenient.}, i.e. the median grade of the alternative $i$, as:
\begin{align}
	f_{maj}(P_i) = \begin{cases}
		r_{(n+1)/2} \text{ when n is odd,} \\
		r_{(n+2)/2} \text{ otherwise.} \commentOC{Maybe it’s just me, but I’d find it more elegant and clear to use a floor (or ceiling) function to define it using only one line.}
	\end{cases}
\end{align}
%i.e. it corresponds to the lower middlemost.
% It could also be written as $f_{maj}(P_i) = \lfloor \frac{n}{2} \rfloor + 1$.
The winner function $F:\Delta^{m} \rightarrow i\in A$ is a function that selects the alternative with the highest median grade as a winner: $w^* = \argmax_{i\in A}f_{maj}(P_i)$. \commentOC{Are you really defining $F$ here? Then $F$ should appear in the left-hand side of this expression. $F(…) = …$. Also, you can’t use $P$ in the definition of $F$ if that is not in the input of $F$, or deducible from the input. Also, this expression defines a set of alternatives, not a single one as the text suggests.} 
In case two or more alternatives have the same highest median grade $\alpha$, the tie is broken by removing one $\alpha$ grade from each of the tied alternatives, recomputing the new median grade and repeating the process until one unique winner is found. \commentOC{This would really deserve a formal presentation, as it is possibly unclear what $\alpha$ denotes here and as I suppose the specific definition will be required later.} We will refer with $F^P$ the result of the function $F(f_{maj}(P))$. \commentOC{That’s the procedure without the tie-breaking application, as you have defined things formally, but the text suggests that $F$ designates the procedure after tie-breaking. Also, even this tie-breaking process does not guarantee unicity: what happens if two alternatives receive exactly the same votes? (There is no neutral function that guarantee unicity.) I’d go for sets of winners.}

\subsection{Incomplete Profile}
In order to analyse the study case described in \Cref{sec:primaire} \commentOC{Aaah that is your goal in this article? (Or part of it?) Then you need to write it in the introduction. And specify better what analyze means (you can leave it a bit fuzzy for now if you are still unsure of the scope, but it should be clarified at some point).} we need to adapt the mechanism to incomplete profiles. Let $\bar{P} \subseteq P$ be our knowledge about the profile $P$. %The voters have full knowledge of their own judgments that we need to elicit starting from zero knowledge.
...
\end{align} 
\commentOC{At this stage it seems to me that you are trying to show something, but you did not tell us what. Please state the claim first.}
\commentOC{Hint: use a latex appropriate command for binomials (I think it’s binom but did not check).}
