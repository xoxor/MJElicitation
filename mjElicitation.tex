...
A \textit{common language} $\Delta = \{ \delta_1, \delta_2, \dots \}$ is a set of strictly ordered grades. It may, or may not, be finite and the notation $\delta_1 \geq \delta_2$ indicates that $\delta_1$ is a better or equivalent grade than $\delta_2$. 
\commentOC{I think that using a symbol different that the usual comparison relation on the natural numbers (perhaps, triangle, see my template) would be most welcome, to reduce possible confusions.}
A profile $P : A\times N \rightarrow \Delta$ is a $m$ by $n$ matrix of grades $P \in \Delta^{A \times N}$. The operator $\rho: \Delta^{N} \rightarrow \Delta^n$ defines an ordering function that given a vector of grades $P_i$ returns the vector ordered by decreasing grades.
...
we denote by $P^S \in \Delta^{S \times N}$ a restriction of the profile $P$ to only the alternatives in $S$, $P^S \subseteq P$. Note that when $S=A$ then $P^S=P$.
...
We define $f: \Delta^{N} \rightarrow \Delta$ a function that assigns to any vector of grades a final grade. Given any $S\subseteq A$, a grading function $f^S: \Delta^{S \times N} \rightarrow \Delta^S$ returns a vector of final grades by applying $f$ to every alternative in $S$.
...
Alternatively, you could give up this part, as your paper is not mainly about how MJ compares to similar rules.}
...
The \emph{majority-grade}, $f_{maj}$, is the function that associates to a vector of grades $q \in \Delta^N$, with $|q|\neq 0$, 
\commentOC{Very minor, but I’d write $\emptyset ≠ q \in \Delta^N$.}
its median grade value: $f_{maj}(q) = \rho(q)_{\lfloor \frac{|q|}{2} \rfloor + 1}$. 
\commentOC{$n$ would be clearer than $\card{q}$. But I suspect that you want to make this more general with $\emptyset ≠ q \in \Delta^N'$ for any $N' \subseteq N$. (And I think that my template has a better floor command.)}
Given any $S\subseteq A$, by applying $f_{maj}$ to all vectors of grades associated to the alternatives in $S$ we obtain the corresponding SGF $f^S_{maj}$. 
\commentOC{Formally, $f^S_\mathit{maj}(P^S)_i = f_\mathit{maj}(P^S_i)$.}
The $i$-\emph{th} element of the resulting vector is the median of the ordered vector of grades associated to the $i$-\emph{th} alternative. Because $f^S_{maj}(P^S)_i$ must depend \commentOC{depends} only on $P^S_i$ we write $f^S_{maj}(P^S_i)$. 
\commentOC{I am not sure I follow. You already have a notation for this, namely, $f_\mathit{maj}(P^S_i)$, isn’t it?}
Moreover, when we consider the complete profile (when $S=A$) we will write $f_{maj}(P_i)$ instead of $f^A_{maj}(P^A_i)$.
\commentOC{I doubt that the $f$ notation is actually useful, it is perhaps short enough to write what it is in full (as you do just below). At least, this mix-up between $f$ and $f^S$ should be clarified.}
%i.e. it corresponds to the lower middlemost.

Given any $S\subseteq A$, the winner function $F^S_{maj}:\Delta^{S \times N} \rightarrow A$ %$F^S_{maj}:\Delta^{S \times N} \rightarrow 2^A \setminus \emptyset$
is a function that selects the alternative with the highest median grade as winner: $F^S_{maj}(P^S) = \argmax_{i\in S}\rho(P^S_i)_{\lfloor \frac{n}{2} \rfloor + 1}$. 
\commentOC{This defines in general a set of alternatives, you can’t just choose to ignore that. You must at least add “assuming it is a singleton”.}
Note that in case $n$ is even two medians could be used for an alternative, but the lower grade is picked.
\commentOC{Perhaps cite the authors again here, to emphasize that this is their choice, if this is true. This remark should come when you define the stuff that picks a median (rho, I believe).}
Let $h$ denote the highest median grade associated to the alternatives in $S$, $h=\max_{i\in S}\rho(P^S_i)_{\lfloor \frac{n}{2} \rfloor + 1}$, and consider the case where two, or more, alternatives could be selected as winners \textemdash i.e. candidates with the same highest median grade $h$. 
\commentOC{Just write “if the set blah is not a singleton, …”}
Ties are broken by removing one $h$ grade from the vectors of grades of each tied alternative, recomputing the new median grade and repeating the process until one unique winner is found or there are no more grades to remove. When $n$ is odd, this is equivalent to take the next element after the median, i.e. the one at index $(\lfloor \frac{n}{2} \rfloor + 1) +1$, if there is still a tie we then look at the previous element before the median, i.e. the one at index $(\lfloor \frac{n}{2} \rfloor + 1) -1$, and keep alternating until the tie is broken or there are no more elements in the vector. When $n$ is even the process is similar but we alternate starting from the element before the median, $\lfloor \frac{n}{2} \rfloor$, then the one after, $\lfloor \frac{n}{2} \rfloor + 2$, and so on.
...
\commentOC{This ordering depends on $P^S$, that’s a bit odd, it should at least be mentioned that it is used when $P^S$ is clear from the context, if this is what you mean. But do you really need this?}
...
Similarly to the winner function, given any $q \in \intvl{1,m}$, we can define a more general \emph{selection function} $F^S_q:\Delta^{S\times N} \rightarrow 2^A \setminus \emptyset$ that selects exactly the $q$ alternatives with the $q$ highest median grades.
Considering that $F^S_{1}=F^S_{maj}$ we define $F^S_{q}=F^S_{q-1} \cup \{i\in S \suchthat i > j, \forall j \in S \setminus F^S_{q-1} \}$.
...
We introduce an additional \emph{Undefined} grade, $\overline{\Delta}=\Delta \cup \{\textit{Undefined}\}$. Voters cannot use this grade to express their judgment over an alternative and it does not count in the computation of the median grade as we will explain formally. We refer to the grades in $\Delta$, the ones used by voters, as "defined" grades.
...
After having asked every voter in $N$ to judge $k$ candidates, we obtain an \emph{incomplete profile} $\overline{P}\in \overline{\Delta}^{A \times N}$, that is a matrix $m \times n$ of grades of which $k \times n$ are "defined". 
\commentOC{Minor, but I’d write $kn$ instead of $k × n$ to avoid a possible confusion in the mind of the reader that this would be a $k × n$ sub-matrix.}
Note that when $k=n$ the resulting profile $\overline{P}$ corresponds to the complete profile $P$. Let $C(\overline{P}) = \{P' \in \Delta^{m \times n} \suchthat \overline{P} \subseteq P'\}$ be the set of all completions of $\overline{P}$ obtained by substituting all \emph{Undefined} grades with "defined" ones, note that $P \in C(\overline{P})$.
...
\commentOC{That some people that are not British choose to write in British English has always puzzled me. That’s a bit like choosing to count in French from France instead of French from Switzerland when one has a choice. Anyway.}
...
Let $g:\overline{\Delta}^N\rightarrow \bigcup_{x \in \intvl{1,n}}\Delta^{x}$, where $x \in \intvl{1,n}$, be a function that given an incomplete vector of grades returns the vector composed only of the $x$ "defined" grades. 
...
Also, specify that given $q \in …$, thus, $q \subseteq N × \Delta bar$, $g(q) = \restr{q}{N × \Delta}$.}
...
Given any $S \subseteq A$, the \emph{majority-grade} for incomplete profile $\overline{f}^S_{maj}: (\bigcup_{x \in \intvl{1,n}}\Delta^{x})^S \rightarrow \Delta^S$ corresponds to $f^S_{maj}$ that only considers the "defined" grades in the computation of the median. Denote by $P'=g(\overline{P}^S_i)$, then $\overline{f}^S_{maj}(P')_i = \rho(P')_{\lfloor \frac{\card{P'}}{2} \rfloor + 1}$, $\forall i \in S$. 
...
Please note that $P^{\tilde{K}}$ is a complete matrix of $k \times n$ grades and that we fall back to the complete profile case, thus, we apply the \emph{majority-grade}, $f^{\tilde{K}}_{maj}$, function to $P^{\tilde{K}}$ to determine the median grades and then the winner function $F^{\tilde{K}}_{maj}$ to select the winner. 
