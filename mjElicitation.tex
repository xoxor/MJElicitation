...
\commentOC{Could also write $\restr{P}{S × N}$, which leads to more generality, if restricting the users is also sometimes convenient.}\commentBN{I'm not sure, I find the notation already a bit heavy and I'm not sure I need a restriction on the voters.}\commentOC{For this definition you do not need $S ≠ \emptyset$, I’d remove this restriction. You could write $\emptyset ≠ S \subseteq A$ instead of restricting on the cardinal.}\commentBN{Ok. But later I use s, I wanted to make sure it's clear what I'm referring to. If it is clear already I agree to remove it.}

Given any $s \in \intvl{1,m}$, a grading function $f^s: \Delta^{s \times n} \rightarrow \Delta^s$ is a function that assigns to any profile $P^S$ a vector of final grades, one for each alternative. 
\commentOC{It seems you never use the identity of the alternatives (the content of $S$), just their number ($s$), thus you could probably simplify by using $s$ instead of $S$ in many places. Independently, the notation $P^S$ is incorrect as used here and later, as it often does not represent a restriction, nor even any combination of a profile $P$ and a set $S$, but simply a matrix in $\Delta^{s × n}$ (which does not require a set $S$ to start with). I’d rather use always the letter $P$ to denote a profile of any size, and write $P \in \Delta^{s × n}$ to specify its size when required. Alternatively, to keep the current approach, you need to always specify that your definitions assume a set $S$ and a profile $P$. That would be (I think) correct, but perhaps misleading, as I am not sure that a complete profile is useful for most of these definitions. I may be wrong, though.}\commentBN{Mmmh idk, maybe? Does using only s guarantee that the alternatives are in A?...does it even matter since it's neutral?}
...
\commentOC{These properties should be defined briefly. Anonymity is built in the definition of a grading function, so I don’t think it can be defined meaningfully here.} 
...
%\end{align}
The \emph{majority-grade}, $f^s_{maj}$, is the SGF that associates to a profile $P^S$ a vector of median grade values, one for each alternative: $f^s_{maj}(P^S)_i = \rho(P^S_i)_{\lfloor \frac{n}{2} \rfloor + 1}$, $\forall i \in S$. 
\commentOC{This requires $n ≠ 0$, which should probably be indicated from the start.}\commentBN{You're right, I wanted to add m and n $>=3$ which I think it's more useful, but I want to make sure it's what I need.}
In other words, the $i$-\emph{th} element of the resulting vector is the median of the ordered vector of grades associated to the alternative $i$. Observe that as $f^s_{maj}$ is IIA, $f^s_{maj}(P^S)_i$ must depend only on $P^S_i$ so we write $f^s_{maj}(P^S_i)$. 
\commentOC{This phrasing is a bit odd: you didn’t prove explicitly that it is IIA. (Though I agree it is more or less obvious so you do not have to prove it explicitly.) And the conclusion is more obvious than the justification: it is immediate from the definition that it depends only on $P^S_i$.}
\commentOC{I’d find it slightly more natural to define first $f…: \Delta^n → \Delta$ and then $f'…$ using that first $f$. Then say that you abuse notation by using the letter $f$ in both cases. This would go from simple to complex rather than the converse.}
Moreover, when we consider the complete profile (when $S=A$) we will write $f_{maj}(P_i)$ instead of $f^m_{maj}(P^A_i)$.
...
Given any $s \in \intvl{1,m}$, the winner function $F^s:\Delta^{s} \rightarrow W \subseteq A$ is a function that selects the alternatives with the highest median grades as winners: 
\commentOC{The codomain is incorrect as $F^s$ selects a set of alternatives, not one alternative (if I understand correctly).}\commentBN{I'm not sure I understand what you mean, W is a subset of the alternatives.}
$F^s(P^S) = \argmax_{i\in S}\rho(P^S_i)_{\lfloor \frac{n}{2} \rfloor + 1}$.
\commentOC{As $S$ is undefined here, you can’t use it when defining $F^s$.}
\commentOC{It is arguable that this is the most natural definition, in case of even $n$, as there are two medians that could be used. I’d find it natural to average them (equivalently, use the other one in case of ties). Is this really the way the authors define it? If so, a short sentence about this would be welcome, to make it clear that this is not an oversight.}\commentBN{Yeah in case of two medians the lower is picked, you're right I should clarify this.}
Let $h$ denote the highest median grade, $h=\max_{i\in S}\rho(P^S_i)_{\lfloor \frac{n}{2} \rfloor + 1}$, and consider the case where two, or more, alternatives are selected as winners \textemdash i.e. candidates with the same highest median grade $h$. Ties are broken by removing one $h$ grade from the vectors of grades of each tied alternative, recomputing the new median grade and repeating the process until one unique winner is found or there are no more grades to remove. 
\commentOC{Isn’t this procedure equivalent to considering the next highest grade, and so on, lexicographically? I think this description is a better explanation of the spirit of the procedure. (And this probably cancels my previous remark about ties.)}\commentBN{Mmh no, unless I'm misreading your comment. In the vectors v=(E,E,E,G,I,I,I) and v1=(E,E,E,G,M,M,M) h=E, if we remove it from both then v=(E,E,G,I,I,I) and v1=(E,E,A,M,M,M) the new medians are I for v and M for v1 (M$>$I), so v1 is selected. The next highest grade would be G for v and A for v1 (G$>$A).}
Please note that in case two, or more, alternatives are associated with the same vector of grades the procedure does not define an unique winner. 
\commentOC{Perhaps you could just define an ordering over the vector of grades. (This may not be a good suggestion, depending on what you want to do with it.)}\commentBN{Like $\rho$?}
We denote by $F^s_{maj}$ the winner function after having applied the aforementioned tie-breaking procedure and, for brevity, we will write $F_{maj}$ when considering $S=A$.
...
\commentOC{Then $F$ is not a function any more, and should be treated as a random variable. I don’t think this is a good idea (depending on what you do with $F$ later). Please try to give up the guarantee on the size of $Q$. If you can’t, you might specify that you always favor “low numbered” alternatives, arbitrarily. But this makes $F$ no more anonymous, and makes sense only if also specifying an order on $M$ and that the alternatives are numbered conforming to that order; which introduces quite a bit of complexity, so might not be really better than the random variable path.}
...
\commentOC{Defining the right, full notations from the start would be more gentle for the reader.} Let $\bar{P}$ be our knowledge about the profile $P$.
...
\commentOC{This is a bit worrysome. It would be nicer to define delta bar from the start and define the median grade procedure exactly, to make it clear how “doesn’t count” is reflected formally (if appropriate).}
...
\commentOC{I think that overline would suit better than bar.}
...
\commentOC{I think that ie requires a comma afterwards, and the spacing could be improved.}
if we ask all voters to judge the same $k$ alternatives, then we have complete knowledge restricted to this set of candidates $K \subseteq A$. As defined in \Cref{sec:complete}, we call $P^{K}$ the restriction of the complete profile $P$ to only the alternatives in $K$.
\commentOC{I wonder if using only one letter ($S$ or $K$) would be clearer.}
...
\commentOC{Indeed, this is incorrect. $d^x$ would perhaps be adequate, depending on what you mean. In any case, $x$ must be bound in the definition.}
...
\commentOC{Given the $P$ vector of grades, define $P'$ the vector with only the defined grades; then define $x$ as its length (this does not deserve a letter, I suppose, as it is simply $\card{P'}$); then take the median. Independently, I think it would be simpler to use only $n$ profiles (rather than $s$ profiles) when allowing Undefined grades.}
...
\commentOC{I do not understand this: how do we know the best $k$ alternatives, before asking?}\commentBN{Then it means I need to be more clear...$\tilde{K}$ is the result of F, so we already asked and already got some answers.}
...
\commentOC{I believe this is not an assumption, as the existence of such a thing is not a hypothesis about the world.}
...
\begin{proposition}
	...
	\commentOC{What is $W$ bar?}
	\commentOC{What is $P^k$? Do you mean a profile restricted to a fixed set of alternatives?}
...
\commentOC{I would not repeat this, it is clear already, and it is strange to repeat it here and not elsewhere. And, you’d need to define truthful.} \commentBN{I mean what I already defined: we assume that they report their preference as they would have stated it when asked about $P$. In other words, $P^{\tilde{K}}_{i} = P_i$ for any $i \in \tilde{K}$.}
...
\begin{itemize}
	\item I'm demoted $\Rightarrow \bar{h}<h$
	\begin{itemize}
		\item others are promoted $\Rightarrow v>\bar{h}$
		\item or not $\Rightarrow v>\bar{h}$
	\end{itemize}
	\item $\bar{h}=h$
	\begin{itemize}
		\item others are promoted $\Rightarrow v>\bar{h}$
	\end{itemize}
	\item I'm promoted $\Rightarrow \bar{h}>h$
	\begin{itemize}
		\item others are promoted $\Rightarrow v>\bar{h}$
	\end{itemize}
\end{itemize}}
...
\commentBN{prob of w to be promoted for $\alpha$ going from $j=0$ to $(n-1)/2$ where: the vector of grades $P_w$ must be composed of at least $\lfloor \frac{n}{2}\rfloor+1$ grades $\alpha > h$ and at most $\lceil \frac{n}{2}\rceil-1$ of grades $\beta \leq h$.}

\[ P(\bar{h}>h)= \largemath{\sum}_{j=0}^{(n-1)/2}{ \ \largemath{\sum}_{x=1}^{n-1}{ \frac{ \largemath{\sum}_{i=0}^{x/2}{ \begin{pmatrix}\frac{n-1}{2}-j \\ {\frac{x+1}{2}+i}\end{pmatrix} \cdot \begin{pmatrix}\frac{n+1}{2}+j \\ {\frac{x-1}{2}-i}\end{pmatrix} }}{\begin{pmatrix}n \\ x\end{pmatrix}} }} \]

\commentBN{$P(\bar{h}=h)= 1-P(\bar{h}>h)\cdot P(\bar{h}<h)$ : probability of being properly labeled}
